...
// 2. Perbaikan Modal Portofolio Pengguna
//    FIX: berkas lama tanpa uploaderUID sekarang dicoba dicocokkan lewat nama
//    pengunggah (uploadedBy) di seluruh node `users`, bukan langsung menyerah.
window.showUserPortfolioModal = function(uploaderUID, uploaderName) {
    const validUID = uploaderUID && uploaderUID !== 'undefined' && uploaderUID !== 'null' && uploaderUID.trim() !== '';

    if (validUID) {
        bukaModalProfilBerdasarkanUID(uploaderUID, uploaderName);
        return;
    }

    // Fallback untuk berkas lama: cari user lain yang nama profilnya cocok
    cariUidBerdasarkanNama(uploaderName).then((hasilUID) => {
        if (hasilUID) {
            bukaModalProfilBerdasarkanUID(hasilUID, uploaderName);
        } else {
            alert(`Profil untuk "${uploaderName}" tidak dapat ditemukan.\n\nCatatan: Berkas ini diunggah sebelum sistem UID aktif (berkas lama) dan tidak ada pengguna lain dengan nama yang cocok persis. Silakan unggah berkas BARU untuk menguji fitur ini!`);
        }
    });
};

// Cari UID dengan mencocokkan nama profil (users/{uid}/profile/name) atau
// nama tampilan berkas lama (uploadedBy) secara case-insensitive.
// Mengembalikan UID jika PERSIS SATU kecocokan ditemukan, selain itu null
// (supaya tidak salah membuka profil orang lain kalau nama sama/ambigu).
function cariUidBerdasarkanNama(namaYangDicari) {
    if (!namaYangDicari) return Promise.resolve(null);

    const usersRef = ref(database, 'users');
    return get(usersRef).then((snapshot) => {
        if (!snapshot.exists()) return null;

        const target = namaYangDicari.trim().toLowerCase();
        const data = snapshot.val();
        const kandidat = [];

        Object.keys(data).forEach((uid) => {
            const profileName = data[uid] && data[uid].profile && data[uid].profile.name;
            if (profileName && profileName.trim().toLowerCase() === target) {
                kandidat.push(uid);
            }
        });

        // Hanya kembalikan hasil kalau kecocokannya tunggal (tidak ambigu)
        return kandidat.length === 1 ? kandidat[0] : null;
    }).catch(() => null);
}

function bukaModalProfilBerdasarkanUID(uid, uploaderName) {
