            ...
            <div class="bg-white p-4 rounded-2xl border border-gray-200 shadow-sm flex items-start justify-between gap-3">
                <div class="min-w-0">
                    ...
                    <div class="text-[10px] font-semibold text-gray-500 mt-0.5">
                        📱 WA: ${v.whatsapp ? escapeHtml(v.whatsapp) : '<span class="italic text-gray-400">nomor default</span>'}
                    </div>
                ...
                <div class="flex flex-col gap-1.5 shrink-0">
                    ...
                    <button onclick="editVoucher('${v.id}', ${v.cost || 0}, ${v.order || 0}, '${escapeHtml(v.whatsapp || '')}')" class="text-[10px] font-bold px-2.5 py-1.5 rounded-lg bg-amber-100 text-amber-700 hover:bg-amber-200">Edit</button>
...
window.addVoucher = function() {
    ...
    set(newRef, { label, cost, order, active: true, whatsapp }).then(() => {
...
window.editVoucher = function(id, currentCost, currentOrder, currentWhatsapp) {
