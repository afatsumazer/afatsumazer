...
// ================= FITUR REDIRECT WHATSAPP (PER-VOUCHER) =================
// Nomor default/fallback dipakai HANYA jika voucher yang diklaim belum diatur
// nomor WA khusus oleh admin (field "whatsapp" di config/vouchers/{id}).
// Disimpan base64 sekadar menghindari nomor tertulis polos di source code —
// ini BUKAN enkripsi asli, siapa pun tetap bisa men-decode base64.
const defaultNomorWaEncoded = "NjI4NTIxNTA3OTMyNA==";

function bersihkanNomorWA(nomor) {
    // Menyisakan digit saja (mendukung input admin dalam format apa pun,
    // misal "+62 852-1507-9324" atau "0852...")
 ...
 * jika ada, redirect langsung ke link itu (Telegram, halaman lain, dll).
 * Jika tidak ada, fallback ke alur WhatsApp seperti biasa.
 *
 * @param {string} namaHadiah
 * @param {number|string} hargaPoin
 * @param {Object} voucherData  Data voucher dari config/vouchers/{id},
...
window.arahkanKlaimHadiah = function(namaHadiah, hargaPoin, voucherData, existingTab, voucherId) {
    ...
    if (linkKustom) {
        ...
        // tanpa membangun pesan WhatsApp sama sekali.
        if (existingTab && !existingTab.closed) {
            existingTab.location.href = linkKustom;
        } else {
            window.open(linkKustom, '_blank');
        }
        return;
    }

    // Tidak ada link kustom -> pakai alur WhatsApp seperti biasa
    window.klaimVoucherKeWhatsApp(namaHadiah, hargaPoin, data.whatsapp || '', existingTab);
};

window.klaimVoucherKeWhatsApp = function(namaHadiah, hargaPoin, nomorTujuan, existingTab) {
    if (!userUID) {
    ...
    // Pakai nomor khusus voucher ini kalau diisi admin, kalau tidak pakai default
    const nomorBersih = bersihkanNomorWA(nomorTujuan);
    const nomorWA = nomorBersih || atob(defaultNomorWaEncoded);

    const linkFinal = `https://wa.me/${nomorWA}?text=${encodedPesan}`;

    if (existingTab && !existingTab.closed) {
        // Arahkan tab yang sudah dibuka sebelumnya (dibuka saat user klik,
        // supaya tidak diblokir sebagai pop-up otomatis oleh browser)
        existingTab.location.href = linkFinal;
    } else {
        // Fallback: buka tab baru langsung (dipakai kalau fungsi ini
        // dipanggil sendiri, langsung dari event klik, tanpa tab bawaan)
    ...
    if (user) {
        ...
        onValue(profileRef, (snapshot) => {
            if (snapshot.exists()) {
                ...
                if (userDisplayEl) userDisplayEl.innerHTML = `<span class="flex items-center">${escapeHtml(displayName)} ${verifiedIcon}</span>`;
                
                const userAvatarEl = document.getElementById('user-avatar');
                if (userAvatarEl) userAvatarEl.src = userPhoto;
                
                if(document.getElementById('profile-card-name')) {
                    document.getElementById('profile-card-name').innerHTML = `<span class="flex items-center justify-center">${escapeHtml(displayName)} ${verifiedIcon}</span>`;
                    document.getElementById('profile-card-avatar').src = userPhoto;
                    document.getElementById('profile-card-email').innerHTML = `
                        <span class="text-indigo-600 font-bold block">${escapeHtml(data.username || '@username')}</span>
                        <span class="text-xs text-gray-500 block mb-2">${escapeHtml(user.email)}</span>
                        ${premiumBadge}
                        <p class="text-sm italic text-gray-600 border-t border-gray-100 pt-2 mt-2">${escapeHtml(data.bio || 'Belum ada bio.')}</p>
                    `;
                }

                // Tampilkan tombol menu Admin hanya untuk akun dengan isAdmin: true
        ...
        // Load lencana (badge) yang diberikan admin ke akun ini
        loadUserBadges();
...
// ================= 1b. LENCANA (BADGE) PEMBERIAN ADMIN =================
function loadUserBadges() {
    if (!userUID) return;
    const badgesRef = ref(database, `users/${userUID}/badges`);
    onValue(badgesRef, (snapshot) => {
        const container = document.getElementById('profile-card-badges');
        if (!container) return;
        ...
        const data = snapshot.val();
        container.innerHTML = Object.values(data).map(b => `
            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold bg-indigo-50 text-indigo-700 border border-indigo-100">
                <span>${escapeHtml(b.icon) || '🏅'}</span><span>${escapeHtml(b.label) || 'Lencana'}</span>
            </span>
        `).join('');
    });
}
...
function loadUserFiles() {
    ...
    onValue(fileRef, (snapshot) => {
        ...
        if (!snapshot.exists()) {
            if (tableBody) tableBody.innerHTML = `<tr><td colspan="2" class="px-6 py-4 text-center text-gray-400 text-xs">Belum ada file di folder <b>${escapeHtml(currentFolder)}</b>.</td></tr>`;
                    ...
                    tr.innerHTML = `
                        <td class="px-6 py-4">
                            <div class="flex items-center space-x-2">
                                <span class="bg-indigo-100 text-indigo-700 text-[10px] font-extrabold px-2 py-0.5 rounded uppercase">${escapeHtml(ext)}</span>
                                <span class="font-medium text-gray-800 break-all text-xs">${escapeHtml(file.name)}</span>
        ...
        if (countInFolder === 0 && tableBody) {
            tableBody.innerHTML = `<tr><td colspan="2" class="px-6 py-4 text-center text-gray-400 text-xs">Belum ada berkas di folder <b>${escapeHtml(currentFolder)}</b>.</td></tr>`;
        }
...
// supaya tidak menyisipkan data besar ke atribut HTML.
window.openFileDetailModal = function(fileKey) {
    ...
    get(sharedRef).then((snapshot) => {
        ...
        let modalEl = document.getElementById('file-detail-modal');
        if (!modalEl) {
            modalEl = document.createElement('div');
            modalEl.id = 'file-detail-modal';
        ...
        modalEl.innerHTML = `
            <div class="bg-white rounded-2xl shadow-xl max-w-sm w-full p-6 relative">
                <button onclick="document.getElementById('file-detail-modal').classList.add('hidden')" class="absolute top-4 right-4 text-gray-400 hover:text-gray-600 bg-gray-100 rounded-full w-8 h-8 flex items-center justify-center font-bold">✕</button>
                <div class="bg-indigo-600 text-white text-xs font-black px-2.5 py-2 rounded-lg shadow-sm uppercase inline-block mb-3">${escapeHtml(ext)}</div>
                <h3 class="text-base font-bold text-gray-900 break-all">${escapeHtml(file.name)}</h3>
                <div class="mt-3 space-y-1 text-xs text-gray-500">
                    <div>Ukuran: <b>${fileSizeStr}</b></div>
                    <div>Diunggah: <b>${pubDateStr}</b></div>
                    <div>Oleh: <b>${escapeHtml(uploaderName)}</b></div>
                </div>
                <button onclick="downloadSharedDirect(null, '${fileKey}')" class="w-full mt-4 bg-indigo-600 hover:bg-indigo-700 text-white py-2 rounded-xl text-xs font-bold transition">
                    Unduh Berkas
                </button>
            </div>
        `;
        modalEl.classList.remove('hidden');
    }).catch(err => alert("Gagal memuat detail berkas: " + err.message));
};
            ...
            Object.keys(data).forEach((key) => {
                ...
                container.innerHTML += `
                    <div class="bg-gray-50 p-3 rounded-lg border border-gray-200 relative group text-left">
                        <h4 class="font-bold text-gray-800 text-xs">${escapeHtml(item.title)}</h4>
                        <p class="text-[11px] text-gray-600 mt-1">${escapeHtml(item.description)}</p>
                        ${item.link ? `<a href="${escapeHtml(item.link)}" target="_blank" class="text-[10px] text-indigo-600 font-semibold mt-1.5 inline-block hover:underline">Lihat Project &rarr;</a>` : ''}
...
window.showUserPortfolioModal = function(uploaderUID, uploaderName) {
    ...
    // Baca "profile" dan "portfolio" SECARA TERPISAH — kedua path ini memang
    // .read: true untuk siapa saja yang login (lihat rules RTDB). Node
    // users/{uid} secara utuh dibatasi hanya untuk pemilik/admin, jadi tidak
    // bisa dibaca langsung di sini kalau yang buka bukan admin.
    const profileRef = ref(database, `users/${uploaderUID}/profile`);
    const portfolioRef = ref(database, `users/${uploaderUID}/portfolio`);

    Promise.all([get(profileRef), get(portfolioRef)]).then(([profileSnap, portfolioSnap]) => {
        let bio = "Belum ada bio.";
        let namaLengkap = uploaderName || "Pengguna";
        let username = "@username";
        let fotoProfil = "https://via.placeholder.com/150";
        ...
        if (profileSnap.exists()) {
            ...
            if (p.bio) bio = p.bio;
            if (p.name) namaLengkap = p.name;
            if (p.username) username = p.username.startsWith('@') ? p.username : `@${p.username}`;
            if (p.photo) fotoProfil = p.photo;
            isVerified = p.isVerified === true;
            isPremium = p.isPremium === true;
        }

        if (portfolioSnap.exists()) {
            const items = Object.values(portfolioSnap.val());
            if (items.length > 0) {
        ...
        const premiumBadge = isPremium ? `
            <div class="mt-2">
                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[9px] font-bold bg-amber-50 text-amber-700 border border-amber-100">
                    ⭐ MEMBER PREMIUM
                </span>
            </div>
        ` : '';
        ...
        modalEl.innerHTML = `
            <div class="bg-white rounded-2xl shadow-xl max-w-md w-full p-6 relative max-h-[90vh] overflow-y-auto">
                ...
                <div class="text-center pb-4 border-b border-gray-100">
                    <img src="${fotoProfil}" alt="Avatar" class="w-20 h-20 rounded-full border-4 border-white shadow-md object-cover mx-auto mb-2">
                    <h3 class="text-lg font-bold text-gray-900 flex items-center justify-center">${escapeHtml(namaLengkap)} ${verifiedIcon}</h3>
                    <p class="text-xs text-indigo-600 font-semibold mt-0.5">${escapeHtml(username)}</p>
                    ${premiumBadge}
                    <p class="text-xs text-gray-500 italic mt-2">"${escapeHtml(bio)}"</p>
                </div>
